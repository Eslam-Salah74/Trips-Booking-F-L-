        ...
        <div class="container">
            <div class="row">
                <div class="col-8 mx-auto text-center">
                    <h2 class="mb-3 text-capitalize">Trips</h2>
                    <ul class="list-inline breadcrumbs text-capitalize" style="font-weight:500">
                        <li class="list-inline-item"><a href="{{route('home')}}" wire:navigate>Home</a>
                        </li>
                        <li class="list-inline-item">/ &nbsp; <a href="{{route('trips')}}" wire:navigate>Trips</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <section class="section">
        <div class="container">
            <div class="row">
                            ...
                            @if ($trips->isNotEmpty())
                                ...
                                <div class="col-md-6" data-aos="fade">
                                    <article class="blog-post">
                                        ...
                                        <div class="pt-4">
                                            <p class="mb-3">{{$trip->city->name}}</p>
                                            <h2 class="h4"><a class="text-black" href="{{route('tripdetails',$trip->id)}}"wire:navigate>{{$trip->tripname}}</a></h2>
                                            <p>{{$trip->subdescription}} …</p> <a href="{{route('tripdetails',$trip->id)}}"wire:navigate class="text-primary fw-bold" aria-label="Read the full article by clicking here">{{$trip->price}} SAR</a>
                                        </div>
                                    </article>
                                </div>
                                @endforeach
                ...
                <div class="col-lg-3">
                    <!-- categories -->
                    <!-- resources/views/livewire/trip-component.blade.php -->
            ...
            <div class="widget widget-categories">
                ...
                <ul>
                    @foreach($categories as $category)
                        <li>
                            <a href="#!" wire:click="filterByCategory({{ $category->id }})" wire:navigate>
                                {{ $category->name }} 
                            ...
                            @if($category->children->isNotEmpty())
                                <ul>
                                    @foreach($category->children as $child)
                                        <li>
                                            <a href="#!" wire:click="filterByCategory({{ $child->id }})" wire:navigate>
                                                {{ $child->name }}
                                                <small class="ml-auto">({{ $child->trips->count() }})</small>
                                            </a>
                                            @if($child->children->isNotEmpty())
                                                <ul>
                                                    @foreach($child->children as $grandchild)
                                                        <li>
                                                            <a href="#!" wire:click="filterByCategory({{ $grandchild->id }})" wire:navigate>
                                                                {{ $grandchild->name }}
                                                                <small class="ml-auto">({{ $grandchild->trips->count() }})</small>
                                                            </a>
                                                        </li>
                                                    @endforeach
                                                </ul>
                    ...
                    <!-- latest post -->
                    <div class="widget">
                        <h5 class="widget-title"><span>Latest Trips</span></h5>
                        @if ($lastThreeTrips->isNotEmpty())
                            <ul class="list-unstyled widget-list">
                                @foreach ($lastThreeTrips as $lastThreeTrip)
                                    <li class="d-flex widget-post align-items-center">
                                        <a class="text-black" href="{{route('tripdetails',$lastThreeTrip->id)}}"wire:navigate>
                                            <div class="widget-post-image flex-shrink-0 me-3">
                                                <img class="rounded" loading="lazy" decoding="async" 
                                                     src="{{ asset('storage/' . $lastThreeTrip->featured_img) }}" 
                                                     alt="{{ $lastThreeTrip->tripname }}">
                                            </div>
                                        </a>
                                        <div class="flex-grow-1">
                                            <h5 class="h6 mb-0">
                                                <a class="text-black" href="{{route('tripdetails',$lastThreeTrip->id)}}"wire:navigate>
                                                    {{ $lastThreeTrip->tripname }}
                                                </a>
                                            </h5>
                                            <small>{{ $lastThreeTrip->created_at->format('F j, Y') }}</small>
                                        </div>
                                    </li>
                                    <br>
                                @endforeach
